...
function loadErrorMessageHandler() {
  ...
  for (var _i = 0; _i < arguments.length; _i++) {
    errorCodes2[_i] = arguments[_i];
  }
  if (!global_default[ApolloErrorMessageHandler]) {
    global_default[ApolloErrorMessageHandler] = handler;
  }
  for (var _a = 0, errorCodes_1 = errorCodes2; _a < errorCodes_1.length; _a++) {
    var codes = errorCodes_1[_a];
    Object.assign(global_default[ApolloErrorMessageHandler], codes);
  }
  return global_default[ApolloErrorMessageHandler];
  function handler(message, args) {
    if (typeof message === "number") {
      var definition = global_default[ApolloErrorMessageHandler][message];
      if (!message || !definition.message)
        return;
      message = definition.message;
    }
    return args.reduce(function(msg, arg) {
      return msg.replace("%s", String(arg));
    }, String(message));
  }
}
...
//# sourceMappingURL=@apollo_client_dev.js.map
